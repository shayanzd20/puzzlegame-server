    ...
    try {
        ...
        if ($this->DBA->Size($UserOrg)) {
            ...
            if ($token == $ValidToken) {
                ...
                $this->users[$from->resourceId]->send(json_encode(array("command" => "notifiUser", "status" => 103, "msg" => MSG_203)));
            }
        } else {
            $this->users[$from->resourceId]->send(json_encode(array("command" => "notifiUser", "status" => 509, "msg" => MSG_209)));
        }
    } catch (Exception $e) {
        $this->users[$from->resourceId]->send(json_encode(array("command" => "notifiUser", "status" => 500, "msg" => $e->getMessage())));
    }
} else {
    $this->users[$from->resourceId]->send(json_encode(array("command" => "notifiUser", "status" => 502, "msg" => MSG_502)));
}
echo "\n--------------------------------END home API:-------------------------------\n";
